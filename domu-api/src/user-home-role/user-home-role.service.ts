...
export class UserHomeRoleService {
  ...
  constructor(
    ...
    @InjectRepository(UserHomeRole)
    private userHomeRoleRepository: Repository<UserHomeRole>,
  ) {}

  async create(createUserHomeRoleDto: CreateUserHomeRoleDto) {
    const user = await this.userRepository.findOneBy({
      id: createUserHomeRoleDto.user_id,
    });
    const home = await this.homeRepository.findOneBy({
      id: createUserHomeRoleDto.home_id,
    });
    const role = await this.roleRepository.findOneBy({
      id: createUserHomeRoleDto.role_id,
    });
    if (!user) {
  ...
  async remove(userId: string, homeId: string) {
    const userHomeRole = await this.userHomeRoleRepository.findOneBy({
      user_id: userId,
      home_id: homeId,
    });
    if (!userHomeRole) {
